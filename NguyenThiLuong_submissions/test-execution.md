...
| TC-10 | REQ-02 — View Book List | Display status "Đã mượn" | Display status "Đang mượn" | Fail |  | BUG-01 |
...
| TC-14 | REQ-03 — Search Books | Show books in category "Công Nghệ" | Does not show the list for the searched category | Fail |  | BUG-02 |
| TC-15 | REQ-03 — Search Books | Show an error message or no data |  | Pass |  |  |
| TC-16 | REQ-03 — Search Books | Show book information for keywords: Technology, Economics | Does not show book information when searching in Filter by Category | Fail |  | BUG-03 |
...
| TC-22 | REQ-04 — Borrow Book | Reject. Show the suspended-account message instead of the expired-account message | Shows the expired-account message instead of the suspended-account message | Fail |  | BUG-04 |
| TC-23 | REQ-04 — Borrow Book | Reject. Show the expired-account message instead of the suspended-account message |  | Pass |  |  |
| TC-24 | REQ-04 — Borrow Book | Reject. Show a message for exceeding the 3-book limit | Borrow succeeds; MEM006 can borrow three more books | Fail |  | BUG-05 |
...
| TC-30 | REQ-05 — Return Book | Return succeeds and an overdue warning is shown | Return succeeds without a warning | Fail |  | BUG-06 |
...
| TC-36 | REQ-05 — Return Book | Reject the return | Return succeeds | Fail |  | BUG-07 |
...
| TC-39 | REQ-06 — Overdue Handling | BR001 displays status "Quá hạn" for MEM002 | BR001 does not display status "Quá hạn" | Fail |  | BUG-08 |
...
| TC-45 | REQ-07 — Member Management | Creation succeeds. The member appears in the list | Rejects with an invalid email message | Fail |  | BUG-09 |
...
| TC-49 | REQ-07 — Member Management | Reject and show an invalid email message | Member creation succeeds | Fail |  | BUG-10 |
...
| TC-52 | REQ-07 — Member Management | Reject and show a phone-number error | Rejects with an invalid email message | Fail |  | BUG-11 |
...
| TC-55 | REQ-08 — Borrow Record Lookup | BR004 does not appear in MEM006's interface | Can view another member's borrow record | Fail |  | BUG-12 |
...
| REQ-06 — Overdue Handling | 8 | 7 | 1 | 87.5% |
| REQ-07 — Member Management | 8 | 5 | 3 | 62.5% |
